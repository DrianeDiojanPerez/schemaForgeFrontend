...
export const TableNode = ({ id, data }: NodeProps<ErdTableNode>) => {
  ...
  return (
    <Card
      className="w-max gap-0 rounded-lg py-0 shadow-lg transition-all hover:shadow-xl"
      style={{ minWidth: TABLE_NODE_WIDTH / 2 }}
    >
      <header
        className={cn(
          "relative flex items-center bg-muted pr-1 pl-2 text-[0.55rem]",
          ITEM_HEIGHT
        )}
      >
      ...
      {data.columns.map((column) => (
        <div
          key={column.id}
          className={cn(
            "relative flex flex-row justify-items-start border-t border-border bg-card text-[8px] leading-5 transition hover:bg-muted",
        ...
        >
          ...
          <div className="flex w-full items-center justify-between gap-3 pr-1">
            ...
            <ColumnTypeCombobox
              nodeId={id}
              value={column.format}
              onValueChange={(format) => saveColumnFormat(column.id, format)}
            />
          </div>
